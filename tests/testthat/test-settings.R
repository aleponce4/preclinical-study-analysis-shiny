testthat::test_that("settings functions save and load presets in an isolated directory", {
  ...
  testthat::expect_equal(loaded$scheduled_sampling_cages$cage_card[[1]], "900010")
...
testthat::test_that("scheduled sampling roles accept friendly labels and YAML-style row lists", {
  normalized <- normalize_scheduled_sampling_cages(list(
    list(cage_card = "900010", role = "Scheduled sampling", censor_day = 6),
    list(cage_card = "900011", role = "survival", censor_day = NA)
  ))

  testthat::expect_equal(normalized$cage_card, c("900010", "900011"))
